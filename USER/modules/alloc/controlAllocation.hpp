...
inline void min_user(const matrix::Matrix<Type, M, N> &x, Type &x_min, size_t &x_index)
{
    const matrix::Matrix<Type, M, N> &self = x;
    x_min = self(0, 0);
    x_index = 0;
...
LinearProgrammingResult<M, N> BoundedRevisedSimplex(LinearProgrammingProblem<M, N> problem) {
    ...
    // inital some value
    matrix::Matrix<float, 1, M> lamt;
    lamt.setZero();
    matrix::Matrix<float, 1, n_m> rdt;
    rdt.setZero();
